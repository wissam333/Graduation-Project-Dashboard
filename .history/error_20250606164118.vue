...
// Dynamic Lottie animations based on error type
const animationData = computed(() => {
  if (error.value?.statusCode === 404) {
    return import("~/assets/lottie/404.json");
  } else if (error.value?.statusCode === 403) {
    return import("~/assets/lottie/403.json");
  } else {
    return import("~/assets/lottie/500.json");
  }
});
