    ...
    <div class="map-wrapper">
      <ClientOnly>
        ...
        >
          ...
          <!-- Step number markers -->
          <LMarker
            v-for="(point, index) in routePointsForLabels"
            :key="index"
...
// Prepare points for step numbers
const routePointsForLabels = computed(() => {
  const points = [];

  // Skip the restaurant (first point) and return to restaurant (last point)
  for (let i = 1; i < routePath.value.length - 1; i++) {
    const position = routePath.value[i];
    points.push({
      position,
      icon: L.divIcon({
        html: `<div style="
...
// Add text to polyline after map is mounted
onMounted(() => {
  nextTick(() => {
    if (map.value?.leafletObject && process.client) {
      ...
      // Add step numbers to the polyline
      const polyline = polyline.value?.leafletObject;
      if (polyline) {
        polyline.setText("   →   ", {
          repeat: true,
          offset: 10,
          attributes: { fill: "#19ad7b", "font-size": "12" },
        });
      }
    }
  });
