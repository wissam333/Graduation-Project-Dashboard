        ...
        <div class="group-selector" v-if="groupedOrders.length > 0">
          ...
          >
            <template #option="slotProps">
              <div
                class="group-option w-100"
                style="text-align: justify"
                :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'"
              >
                <div>
                  <Icon class="icon" name="mdi:map-marker-path" />
                  <span class="group-label mx-2">{{ slotProps.option.label }}</span>
                </div>
                <small class="group-time">{{ slotProps.option.time }}</small>
              </div>
            </template>
          </Dropdown>
        </div>
...
const acceptGroupRoute = async (grouping) => {
  try {
    const orderIds = grouping.orders.map((o) => o.order._id);
...
const showSelectedGroup = () => {
  if (selectedGroupIndex.value !== null) {
    selectedGrouping.value =
      groupedOrders.value[selectedGroupIndex.value.value];
