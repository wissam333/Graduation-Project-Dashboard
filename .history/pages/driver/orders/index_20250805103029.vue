<template>
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <button class="action-btn btn-primary" @click="getGroupedOrders">
          <Icon class="icon" name="mdi:map-marker-path" />
          {{ $i18n.locale === "ar" ? "مسارات مقترحة" : "Suggested Routes" }}
        </button>
      </div>
    </div>
  ...
  <driverMapDriverRouteMap
    v-if="showRouteMap && selectedGrouping && restaurant"
...
const getGroupedOrders = async () => {
  try {
    ...
    const { data } = await useFetch(`${api.OrdersApi}/suggestOrderGroupings`, {
      baseURL: apiBase,
      query: {
        restaurantId: useUserInfo().value?.restaurantId, // Replace with actual restaurant ID
        maxGroupSize: 3,
        maxDistanceKm: 5,
      },
    });
    ...
    if (data.value && data.value.groupings.length > 0) {
      ...
      selectedGrouping.value = groupedOrders.value[0]; // Select first grouping by default
      showRouteMap.value = true;
    } else {
      $awn.alert(
        locale.value === "en"
          ? "No suggested routes available"
          : "لا توجد مسارات مقترحة متاحة"
      );
    }
  } catch (error) {
