  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      <div class="head-title">
        <h4 class="header4">
          {{ $i18n.locale === "ar" ? "مستحقات السائقين" : "Driver Dues" }}
        </h4>
      </div>
      <div class="options">
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>
                {{ $i18n.locale === "ar" ? "السائق" : "Driver" }}
              </th>
              <th>
                {{ $i18n.locale === "ar" ? "المستحقات" : "Dues" }}
              </th>
              <th>
              ...
              <td class="action-cell">
                <div class="d-flex">
                  ...
                  <button
                    ...
                    class="edit-btn delete-btn"
                    @click="updateOrderStatusToDone(order._id)"
                    v-tooltip.left="
                      $i18n.locale === 'ar' ? 'تم الدفع' : 'Payment Done'
                    "
                  >
...
<script setup>
definePageMeta({
  middleware: ["not-authorize", "admin"], // Changed to driver middleware
});
...
const filteredArray = computed(() => {
  ...
  if (restaurantFilter.value && restaurantFilter.value._id) {
    filtered = filtered.filter(
      (cat) => cat.driver?.restaurantId === restaurantFilter.value._id
    );
  }
...
const getOrders = async () => {
  ...
  try {
    const { data: ordersData } = await useFetch(
      `${api.OrdersApi}/getDriverDues`, // Only fetch Processing orders
      {
        baseURL: apiBase,
        headers: {
...
const updateOrderStatusToDone = async (currentOrderId) => {
  try {
    await useFetch(
      `${api.OrdersApi}/updateDriverDuesStatus/${currentOrderId}`,
      {
        baseURL: apiBase,
        method: "PUT",
        body: {
