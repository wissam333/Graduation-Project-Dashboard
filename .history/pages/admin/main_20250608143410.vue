...
watch(selectedPeriod, (newPeriod) => {
  fetchSalesData(newPeriod.value);
});

// Fetch all dashboard data
const fetchDashboardData = async () => {
  try {
    const [statsRes, productsRes, statusRes, categoriesRes, comparisonRes] =
      await Promise.all([
        ...
        useFetch("/api/dashboard/comparison?type=month", {
          baseURL: apiBase,
          headers: {
            token: `Bearer ${useUserInfo().value.accessToken}`,
          },
    ...
    if (statsRes.data.value) {
      summaryData.value = statsRes.data.value;
      if (statsRes.data.value.popularCategories) {
        setCategoriesData(statsRes.data.value.popularCategories);
      }
    }
    ...
    fetchSalesData(selectedPeriod.value.value);
  } catch (error) {
    console.error("Error fetching dashboard data:", error);
  }
};

// Fetch sales data separately (called on init and when period changes)
