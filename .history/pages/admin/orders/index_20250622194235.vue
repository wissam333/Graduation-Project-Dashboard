  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    ...
    <div class="table-responsive">
      ...
      <div v-else class="user-table">
        ...
        <div class="pagination" v-if="filteredArray.length > 0">
          <button
            class="pagination-btn"
            @click="changePage(currentPage - 1)"
            :disabled="currentPage == 1"
          >
            &laquo; {{ $i18n.locale === "ar" ? "السابق" : "Previous" }}
          </button>
          ...
          >
            {{ $i18n.locale === "ar" ? "التالي" : "Next" }} &raquo;
          </button>
        </div>
      </div>
    </div>
...
const changePage = (page) => {
  ...
  if (page !== currentPage.value) {
    ...
    getOrders(); // Make sure this is called to fetch new data
  }
};
...
watch([restaurantFilter, clientsFilter, searchText], () => {
  currentPage.value = 1;
});
...
const getOrders = async () => {
  ...
  try {
    ...
    if (restaurantFilter.value?._id) {
      query.restaurantId = restaurantFilter.value._id;
    }
    if (clientsFilter.value?._id) {
      query.userId = clientsFilter.value._id;
    }
    ...
    if (ordersData.value) {
      ...
      totalItems.value = ordersData.value.pagination.total;
    }
  } catch (error) {
    console.error("Error fetching orders:", error);
    $awn.alert(locale.value === "en" ? "Failed to load orders" : "فشل تحميل قائمة الطلبات");
  } finally {
    loading.value = false;
  }
};
