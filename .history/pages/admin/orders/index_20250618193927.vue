      ...
      <div v-else class="user-table">
        <table>
          ...
          <tbody>
            <tr
              class="table-row"
              v-for="(order, index) in filteredArray"
              :key="order._id"
              :class="{ grobed: order.isGroupedOrder }"
              :style="grobedStyle(index)"
            >
              <td class="User-cell">
                <div class="User-info">
                  <h5>{{ order.name }}</h5>
...
const grobedStyle = (index) => {
  if (!this.item.grobed) return {};

  // Calculate hue based on index (0-360)
  const hue = (index * 30) % 360; // Adjust multiplier for more/less variation
  return {
    "--border-color": `hsl(${hue}, 75%, 60%)`, // Adjust saturation & lightness as needed
  };
};
</script>
