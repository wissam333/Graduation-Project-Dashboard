<template>
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    ...
    <div class="table-responsive">
      ...
      <div class="pagination-container" v-if="!loading && filteredArray.length">
        <Paginator
          v-model="currentPage"
          :rows="itemsPerPage"
        ...
        >
          <template #start>
            <span class="pagination-info">
              {{ $i18n.locale === "ar" ? "الصفحة الحالية" : "Current Page" }}:
              {{ currentPage }}
            </span>
          </template>
        </Paginator>
...
const currentPageReportTemplate = computed(() => {
  return locale.value === "ar"
    ? "عرض {first} إلى {last} من {totalRecords} عناصر"
...
const { data: clients } = await useFetch(`${api.UsersApi}/temp`, {
  baseURL: apiBase,
});
...
const filteredArray = computed(() => {
  ...
  if (restaurantFilter.value?._id) {
    const restaurantId = restaurantFilter.value._id;
    filtered = filtered.filter((order) => {
      const products = order.products;
      if (!products?.length) return false;
      return products.some((meal) => {
        const product = meal.product;
        return product?.restaurantId?._id === restaurantId;
      });
    });
  }
  ...
  // Update total items count for client-side pagination
  totalItems.value = filtered.length;
  
  // If doing client-side pagination (uncomment if needed)
...
const getOrders = async (page = 1) => {
  ...
  try {
    ...
    if (ordersData.value) {
      ...
      currentPage.value = ordersData.value.pagination.currentPage;
      itemsPerPage.value = ordersData.value.pagination.itemsPerPage;
      totalItems.value = ordersData.value.pagination.totalItems;
      totalPages.value = ordersData.value.pagination.totalPages;
    }
  } catch (error) {
...
const onPageChange = (event) => {
  currentPage.value = event.page + 1; // Paginator uses 0-based index
