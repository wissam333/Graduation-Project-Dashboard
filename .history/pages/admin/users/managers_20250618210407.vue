    ...
    <template #container="{ closeCallback }">
      ...
      >
        ...
        <div class="dialog-content">
          ...
          <Select
            v-model="editSelectedRestaurant"
            :options="restaurants"
            :placeholder="
              $i18n.locale === 'ar' ? 'اختار مطعم' : 'Select A Restaurant'
            "
...
const editSelectedRestaurant = ref(null);
...
const getManagers = async () => {
  loading.value = true;
  try {
    const { data: UsersData } = await useFetch(`${api.UsersApi}?role=2`, {
      baseURL: apiBase,
      headers: {
    ...
    if (UsersData.value) {
      Managers.value = UsersData.value.filter((user) => user.role === 2);
    }
  } catch (error) {
  ...
  try {
    await useFetch(api.CreateManager, {
      ...
      body: {
        ...
        email: email.value,
        restaurantId: selectedRestaurant.value._id,
      },
    });
...
const openEditDialog = (manager) => {
  ...
  editPassword.value = "";
  editSelectedRestaurant.value = manager.restaurant || null;
...
const updateManager = async () => {
  try {
    ...
    if (editSelectedRestaurant.value) {
      body.restaurantId = editSelectedRestaurant.value._id;
    }
    ...
    editEmail.value = "";
    editPassword.value = "";
    editSelectedRestaurant.value = null;
