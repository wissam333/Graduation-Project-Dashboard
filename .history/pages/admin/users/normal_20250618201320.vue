    ...
    <div class="table-responsive">
      ...
      <div v-else class="user-table">
        <table>
          ...
          <tbody>
            <tr class="table-row" v-for="user in filteredUsers" :key="user._id">
              <td class="User-cell">
...
const getUsers = async () => {
  loading.value = true;
  try {
    const { data: UsersData } = await useFetch(`${api.UsersApi}?role=1`, {
      baseURL: apiBase,
      headers: {
...
const exportToExcel = () => {
  const filteredData = filteredUsers.value;
  const data =
    locale.value === "en"
