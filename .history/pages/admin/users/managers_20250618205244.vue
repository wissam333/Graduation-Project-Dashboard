...
const deleteManager = async () => {
  try {
    const { data } = await useFetch(
      `${api.UsersApi}/${deletedManager.value._id}`,
      {
        baseURL: apiBase,
        method: "DELETE",
      }
...
const addManager = async () => {
  try {
    const { data } = await useFetch(api.CreateManager, {
      baseURL: apiBase,
      method: "POST",
      body: {
...
const updateManager = async () => {
  try {
    ...
    const { data } = await useFetch(
      `${api.UsersApi}/${currentManagerId.value}`,
      {
        baseURL: apiBase,
        method: "PUT",
        body: body,
...
const toggleManagerStatus = async (manager) => {
  try {
    const { data } = await useFetch(
      `${api.UsersApi}/toggle-status/${manager._id}`,
      {
        baseURL: apiBase,
        method: "PUT",
        headers: {
