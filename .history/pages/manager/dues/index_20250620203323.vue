          ...
          <tbody>
            ...
            >
              ...
              <td class="action-cell">
                <div class="d-flex">
                  <nuxt-link
                    :to="`/manager/dues/${order.order._id}`"
...
<script setup>
definePageMeta({
  middleware: ["not-authorize", "manager"], // Changed to driver middleware
});
...
const getOrders = async () => {
  ...
  try {
    const { data: ordersData } = await useFetch(
      `${api.OrdersApi}/getDriverDues?restaurantId=${useUserInfo().value.restaurantId}`, // Only fetch Processing orders
      {
        baseURL: apiBase,
        headers: {
