    ...
    <template #container="{ closeCallback }">
      ...
      >
        ...
        <div class="dialog-footer">
          <button class="btn btn-secondary" @click.stop="closeAddDialog">
            {{ $t("Close") }}
          </button>
    ...
    <template #container="{ closeCallback }">
      ...
      >
        ...
        <div class="dialog-content">
          ...
          <div class="restaurant-info mb-4" v-if="editCategory.restaurantId">
            <label class="form-label">
              {{ $i18n.locale === "ar" ? "المطعم" : "Restaurant" }}
            </label>
            <div class="restaurant-value">
              {{ getRestaurantName(editCategory.restaurantId) }}
            </div>
          </div>
        ...
        <div class="dialog-footer">
          <button class="btn btn-secondary" @click.stop="closeEditDialog">
            {{ $t("Close") }}
          </button>
          <button class="btn btn-success" type="submit">
            {{ $t("Update") }}
...
const fetchRestaurants = async () => {
  try {
    const { data } = await useFetch(`${api.RestaurantsApi}`, {
      baseURL: apiBase,
    });
  ...
  try {
    await useFetch(`${api.CategoriesApi}/${editCategory.value._id}`, {
      ...
      body: {
        ...
        restaurantId: editCategory.value.restaurantId,
      },
    });
