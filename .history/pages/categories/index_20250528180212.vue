    ...
    <div class="title-container">
      ...
      <div class="options">
        ...
        <button
          ...
          @click="restaurantFilter = null"
          v-if="restaurantFilter"
        >
          <Icon class="icon" name="material-symbols:close" />
          {{ $i18n.locale === "ar" ? "مسح الفلتر" : "Clear Filter" }}
        </button>
      </div>
    </div>
    ...
    <template #container="{ closeCallback }">
      ...
      >
        ...
        <div class="dialog-content">
          ...
          <Select
            v-model="newCategory.restaurant"
            :options="restaurants"
            :placeholder="
              $i18n.locale === 'ar' ? 'اختار مطعم' : 'Select A Restaurant'
            "
...
const restaurantsWithAllOption = computed(() => {
  if (restaurants.value) {
    return [{ _id: null, nameAr: "الكل", nameEn: "All" }, ...restaurants.value];
  }
});
...
const addCategory = async () => {
  try {
    ...
    formData.append("restaurantId", newCategory.value.restaurant._id);
    if (newCategory.value.imageFile) {
      formData.append("img", newCategory.value.imageFile);
    }
...
const updateCategory = async () => {
  try {
    ...
    formData.append("restaurantId", editCategory.value.restaurant._id);
    if (editCategory.value.imageFile) {
      formData.append("img", editCategory.value.imageFile);
    }
