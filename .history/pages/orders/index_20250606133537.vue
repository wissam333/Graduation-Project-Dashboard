  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <!-- Restaurant Filter -->
        <button
          class="action-btn btn-filter"
          @click.prevent="toggleRestaurantDropdown"
          :disabled="loadingRestaurants"
        >
          <template v-if="loadingRestaurants">
        ...
        <div v-if="showRestaurantDropdown" class="dropdown-menu">
          <input
            v-model="restaurantSearch"
            :placeholder="
              $i18n.locale === 'ar' ? 'ابحث عن مطعم' : 'Search restaurant'
            "
        ...
        <div class="dropdown">
          ...
          <div v-if="showClientDropdown" class="dropdown-menu">
            <input
              v-model="clientSearch"
              :placeholder="
                $i18n.locale === 'ar' ? 'ابحث عن عميل' : 'Search client'
              "
              class="dropdown-search"
              @input="filterClients"
            />
          ...
          <button
            v-if="selectedRestaurant || selectedClient"
            class="action-btn btn-clear"
            @click="clearFilters"
          >
            <Icon name="mdi:close" class="icon" />
            {{ $i18n.locale === "ar" ? "مسح الفلاتر" : "Clear filters" }}
          </button>
        </div>
...
const showRestaurantDropdown = ref(false);
...
const fetchRestaurants = async () => {
  ...
  try {
    ...
    if (restaurantsData.value) {
      allRestaurants.value = restaurantsData.value;
...
const fetchClients = async () => {
  ...
  try {
    const { data: clientsData } = await useFetch(`${api.ClientsApi}/temp`, {
      baseURL: apiBase,
      headers: {
        "accept-language": locale.value,
      },
    ...
    if (clientsData.value) {
      allClients.value = clientsData.value;
      filteredClients.value = clientsData.value;
    }
  } catch (error) {
...
const filterRestaurants = () => {
  if (!restaurantSearch.value) {
    filteredRestaurants.value = allRestaurants.value;
    return;
  }
  filteredRestaurants.value = allRestaurants.value.filter((restaurant) =>
...
const filterClients = () => {
  ...
  filteredClients.value = allClients.value.filter(
    (client) =>
...
// Toggle dropdowns and fetch data if needed
const toggleRestaurantDropdown = async () => {
  showRestaurantDropdown.value = !showRestaurantDropdown.value;
  showClientDropdown.value = false;

  if (showRestaurantDropdown.value && allRestaurants.value.length === 0) {
    await fetchRestaurants();
  }
};

const toggleClientDropdown = async () => {
  showClientDropdown.value = !showClientDropdown.value;
  showRestaurantDropdown.value = false;

  if (showClientDropdown.value && allClients.value.length === 0) {
    await fetchClients();
  }
};
...
// Update filteredArray to consider the filters
const filteredArray = computed(() => {
  return orders.value?.filter((order) => {
    ...
    const matchesRestaurant =
      ...
      order.products?.some(
        (product) =>
          product.product?.restaurantId === selectedRestaurant.value._id
      );

    // Client filter
    const matchesClient =
      !selectedClient.value || order.userId === selectedClient.value._id;

    return matchesSearch && matchesRestaurant && matchesClient;
  });
});
