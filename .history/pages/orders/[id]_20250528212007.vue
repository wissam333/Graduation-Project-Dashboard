...
// Print order with better formatting
const printOrder = () => {
  // Create a new window for printing
  const printWindow = window.open("", "_blank");

  // Get the current date for the print header
  const printDate = new Date().toLocaleDateString(locale.value, {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  let printContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>${locale.value === "ar" ? "تفاصيل الطلب" : "Order Details"} - ${
    order.value._id
  }</title>
      <style>
        body {
        ...
        .print-header {
          text-align: center;
          margin-bottom: 20px;
          border-bottom: 2px solid #19ad7b;
          padding-bottom: 10px;
        }
        .print-header h1 {
          color: #19ad7b;
    ...
    <body ${locale.value === "ar" ? 'dir="rtl"' : ""}>
      <div class="print-header">
        <h1>${locale.value === "ar" ? "تفاصيل الطلب" : "Order Details"}</h1>
        <p>${printDate}</p>
      </div>
  `;
  ...
  printContent += `
    <div class="print-section">
      <div class="info-grid">
        <div class="info-card">
          <h2>${
            locale.value === "ar" ? "معلومات الطلب" : "Order Information"
          }</h2>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "رقم الطلب" : "Order ID"
            }:</span>
            <span>${order.value._id}</span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "التاريخ" : "Date"
            }:</span>
            <span>${formatDate(order.value.createdAt)}</span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "الحالة" : "Status"
            }:</span>
            <span class="status-badge ${statusClass(order.value.status)}">
              ${
                locale.value === "ar"
                  ? getStatusArabic(order.value.status)
                  : order.value.status
              }
            </span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "المبلغ الإجمالي" : "Total Amount"
            }:</span>
            <span>${order.value.amount} ${
    locale.value === "ar" ? "ل.س" : "SYP"
  }</span>
          </div>
        </div>
        ...
        <div class="info-card">
          <h2>${
            locale.value === "ar" ? "معلومات العميل" : "Customer Information"
          }</h2>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "الاسم" : "Name"
            }:</span>
            <span>${order.value.name}</span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "البريد الإلكتروني" : "Email"
            }:</span>
            <span>${order.value.email}</span>
          </div>
        </div>

        <div class="info-card">
          <h2>${
            locale.value === "ar" ? "معلومات التوصيل" : "Delivery Information"
          }</h2>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "العنوان" : "Address"
            }:</span>
            <span>${order.value.address}</span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "موقع التوصيل" : "Delivery Location"
            }:</span>
            <span>${address.value}</span>
          </div>
          <div class="info-row">
            <span class="info-label">${
              locale.value === "ar" ? "تكلفة التوصيل" : "Delivery Price"
            }:</span>
            <span>${order.value.deliveryPrice} ${
    locale.value === "ar" ? "ل.س" : "SYP"
  }</span>
          </div>
        </div>
  ...
  printContent += `
    <div class="print-section">
      <h2>${locale.value === "ar" ? "المنتجات" : "Products"}</h2>
      <table>
        <thead>
          <tr>
            <th>${locale.value === "ar" ? "المنتج" : "Product"}</th>
            <th>${locale.value === "ar" ? "المطعم" : "Restaurant"}</th>
            <th class="text-right">${
              locale.value === "ar" ? "السعر" : "Price"
            }</th>
            <th class="text-center">${
              locale.value === "ar" ? "الكمية" : "Qty"
            }</th>
            <th class="text-right">${
              locale.value === "ar" ? "المجموع" : "Total"
            }</th>
          </tr>
        </thead>
        <tbody>
      ...
      <tr>
        ...
        <td>${
          item.product?.restaurantId
            ? getRestaurantName(item.product.restaurantId)
            : "-"
        }</td>
        <td class="text-right">${item.product?.price || 0} ${
      locale.value === "ar" ? "ل.س" : "SYP"
    }</td>
        <td class="text-center">${item.quantity}</td>
        <td class="text-right">${(item.product?.price || 0) * item.quantity} ${
      locale.value === "ar" ? "ل.س" : "SYP"
    }</td>
      </tr>
    `;
  ...
  printContent += `
          <tr class="total-row">
            <td colspan="4" class="text-right">${
              locale.value === "ar" ? "مجموع المنتجات" : "Products Subtotal"
            }:</td>
            <td class="text-right">${
              order.value.amount - order.value.deliveryPrice
            } ${locale.value === "ar" ? "ل.س" : "SYP"}</td>
          </tr>
          <tr class="total-row">
            <td colspan="4" class="text-right">${
              locale.value === "ar" ? "تكلفة التوصيل" : "Delivery Price"
            }:</td>
            <td class="text-right">${order.value.deliveryPrice} ${
    locale.value === "ar" ? "ل.س" : "SYP"
  }</td>
          </tr>
          <tr class="total-row">
            ...
            <td class="text-right">${order.value.amount} ${
    locale.value === "ar" ? "ل.س" : "SYP"
  }</td>
          </tr>
        </tbody>
