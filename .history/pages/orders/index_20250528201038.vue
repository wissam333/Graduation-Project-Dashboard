        ...
        <table>
          <thead>
            <tr>
              ...
              <th>{{ $i18n.locale === "ar" ? "المنتجات" : "Products" }}</th>
              <th>{{ $i18n.locale === "ar" ? "المبلغ" : "Amount" }}</th>
          ...
          <tbody>
            <tr class="table-row" v-for="order in filteredArray" :key="order._id">
              <td class="User-cell">
                <div class="User-info">
                  <h5>{{ order.name }}</h5>
              ...
              <td class="products-cell">
                <div v-for="product in order.products" :key="product.productId" class="product-item">
                  <span v-if="product.product">{{ product.product.title }} (x{{ product.quantity }})</span>
                  <span v-else>{{ product.productId }} (x{{ product.quantity }})</span>
                </div>
              </td>
              ...
              <td class="status-cell">
                <span class="badge" :class="statusClass(order.status)">
                  {{ $i18n.locale === "ar" ? getStatusArabic(order.status) : order.status }}
                </span>
              </td>
              <td class="date-cell">
                <span class="badge">
              ...
              <td class="action-cell">
                <button class="icon-btn edit-btn" @click="openEditDialog(order)">
  ...
  <ElementsAppModal :isOpen="deleteModal" @updateIsOpen="updateDeleteModal">
    ...
    <template #content>
      <p>
        ...
        {{ $i18n.locale === "ar" ? "هذا الطلب" : "this order" }} (ID: {{ deleteOrder._id }}){{ $i18n.locale === "ar" ? "؟" : "?" }}
      </p>
    </template>
    <template #footer>
        ...
        <div class="dialog-content">
          <div class="input-group">
            ...
            <select
              ...
              class="form-input"
            >
              <option value="pending">{{ $i18n.locale === "ar" ? "قيد الانتظار" : "Pending" }}</option>
              <option value="processing">{{ $i18n.locale === "ar" ? "قيد التجهيز" : "Processing" }}</option>
              <option value="shipped">{{ $i18n.locale === "ar" ? "تم الشحن" : "Shipped" }}</option>
              <option value="delivered">{{ $i18n.locale === "ar" ? "تم التوصيل" : "Delivered" }}</option>
              <option value="cancelled">{{ $i18n.locale === "ar" ? "ملغى" : "Cancelled" }}</option>
            </select>
          </div>
...
if (process.client) {
  const suc = (res) => {
    if (res.coords.latitude && res.coords.longitude) {
      //marker
      editMarker.value.latitude = res.coords.latitude;
      editMarker.value.longitude = res.coords.longitude;
    }
  };
...
const filteredArray = computed(() => {
  return orders.value?.filter((order) =>
    ...
    order._id.toLowerCase().includes(searchText.value.toLowerCase()) ||
    order.status.toLowerCase().includes(searchText.value.toLowerCase())
  );
});
...
const exportToExcel = () => {
  const dataToExport = filteredArray.value.map((order) => {
    ...
    const baseData = {
      ...
      customer: order.name,
      email: order.email,
      products: order.products.map(p => `${p.product?.title || p.productId} (x${p.quantity})`).join(', '),
      amount: order.amount,
