  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <button class="action-btn btn-success" @click="getCustomers()">
    ...
    <div class="table-responsive">
      <div class="loading" v-if="loading">
        <img src="/loading/loadingnew2.svg" alt="Loading..." />
      </div>
    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
        ...
        <div class="dialog-footer">
          ...
          <button class="btn btn-success" @click="addCustomer()" autofocus>
            {{ $t("Add") }}
          </button>
        </div>
      </div>
...
const getCustomers = async () => {
  loading.value = true;
  try {
    const { data: customersData } = await useFetch(`${api.GetAllCustomers}`, {
      credentials: "include",
      baseURL: apiBase,
      headers: {
...
const DeleteCat = async () => {
  try {
    await useFetch(`${api.DeleteCustomer}/${deleteCustomer.value.id}`, {
      credentials: "include",
      baseURL: apiBase,
      method: "DELETE",
    });
    ...
    $awn.alert(
      locale.value === "en" ? "Failed to delete customer" : "فشل حذف الزبون"
    );
  }
};
...
const addCustomer = async () => {
  try {
    ...
    $awn.alert(
      locale.value === "en" ? "Failed to add customer" : "فشل إضافة الزبون"
    );
  }
};
