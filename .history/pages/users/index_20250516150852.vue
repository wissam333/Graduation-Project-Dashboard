    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
        ...
        <div class="dialog-footer">
          ...
          <button class="btn btn-success" @click="addManager()" autofocus>
            {{ $t("Add") }}
          </button>
        </div>
      </div>
...
const { data: RestaurantData } = await useFetch(`${api.RestaurantsApi}`, {
  baseURL: apiBase,
});

watchEffect(() => {
  if (process.client && RestaurantData.value) {
    restaurants.value = RestaurantData.value;
  }
});
...
const DeleteCat = async () => {
  try {
    await useFetch(`${api.UsersApi}/${deleteUser.value.id}`, {
      credentials: "include",
      baseURL: apiBase,
      method: "DELETE",
    });
